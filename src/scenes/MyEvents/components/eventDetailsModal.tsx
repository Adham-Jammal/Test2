...
class EventDetails extends AppComponentBase<EventDetailsModalProps, any> {
  ...
  paginationOptions = {
    ...
    onShowSizeChange: async (page: any, pageSize: any) => {
      ...
      this.setState(temp);
      await this.updateSchedulesList(pageSize, 0, this.props.eventData?.id!);
    },
  ...
  eventsSchedulesTableColumns = [
    ...
    },
    {
      ...
      render: (_: any, item: any) => {
        return `${moment(item.startDate).format(timingHelper.defaultDateFormat)}-${
          item.fromHour
        } ${L('To')} ${moment(item.endDate).format(timingHelper.defaultDateFormat)}-${
          item.toHour
        } `;
      },
    },
