...
const SortEventOrders = (props:any) => {
    ...
    const SortableItem = SortableElement(({value}:any) => <li><span>{value.name}</span></li>);
    const SortableList = SortableContainer(({items}:any) => {
        if(items.length === 0)
            setItems(events);
        return (
            <ol className="events-imgs">
            {items.map((value:any, index:number) => <SortableItem key={`item-${value.id}`}  index={index} value={value} /> )}
            </ol>
        );
    });
